...
import semi.beans.dto.ReviewInfoGetDto;
...
public class ReviewDao {
	...
	public void write(ReviewDto revdto) throws Exception {
		...
		ps.setInt(4, revdto.getReview_res_no());
		ps.setString(5, revdto.getReview_title());
		ps.setString(6, revdto.getReview_content());
		ps.execute();
	...
	// 펜션 별 리뷰 검색 리스트
	public List<ReviewDto> search(int review_pension_no, String type, String keyword, int start, int finish)
			throws Exception {
		Connection con = getConnection();
	...
	// 리뷰번호 예약자번호 반환 메소드
	public ReviewInfoGetDto getInfo(int member_no,int pension_no) throws Exception {
		Connection con = getConnection();
		String sql = "SELECT PENSION_NAME ,r.ROOM_NAME ,res.* FROM PENSION LEFT OUTER JOIN ROOM r ON PENSION_NO =r.ROOM_PENSION_NO LEFT OUTER JOIN RESERVATION res ON r.ROOM_NO =res.RES_ROOM_NO WHERE res.RES_WRITE =? AND PENSION_NO =?";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, member_no);
		ps.setInt(2,pension_no);
		ResultSet rs = ps.executeQuery();
		ReviewInfoGetDto rdto;
		if(rs.next()) {
			rdto = new ReviewInfoGetDto();
			rdto.setRes_no(rs.getInt("res_no"));
			rdto.setRes_write(rs.getInt("res_write"));
		}else {
			rdto = null;
		}
		con.close();
